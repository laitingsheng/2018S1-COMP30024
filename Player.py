...
class Player:
    ...
    def _eval_move(self, board):
        '''
        evaluate the state of the board from perspective of player
        himself/herself at the moving phases

        basically according to:
            1) the number of pieces left
            2) the position of the pieces
            3) the potentially surrouned pieces in moving phases
        '''
        if board.n_pieces[self.oppo] < 2:
            return inf
        mine = board.n_pieces[self.mine]
        oppo = board.n_pieces[self.oppo]
        re = cm1 * (mine - oppo)
        if mine > oppo:
            re *= (mine + 1) / (oppo + 1)
        else:
    ...
    def _eval_place(self, board):
        '''
        evaluate the state of the board from perspective of player
        himself/herself at the placing phases

        basically according to:
            1) the number of pieces left
            2) the position of the pieces
            3) the potentially surrouned pieces
        '''
    ...
    def _move(self):
        '''
        make the move generate by the minMax algorithm
        '''
        board = self.board
    ...
    def _move_max(self, board, depth, alpha, beta):
        '''
        return the maximum score for the player that can be reached by the
        current moves
        '''
    ...
    def _move_min(self, board, depth, alpha, beta):
        '''
        return the minimum score for the player that can be reached by the
        current moves
        '''
        if depth == self.depth or board.end():
            return self._eval_move(board)
    ...
    def _place(self):
        '''
        place the piece according to the position generated by minMax
        '''
        board = self.board
        mine = self.mine
    ...
    def _reachable(self, board, player, x, y, used_piece):
        '''
        this function checks if the given player can reach the given position
        with pieces other than the used_piece (the used piece is used to
        surround the enermy from the other side)
        '''
